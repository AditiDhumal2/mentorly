/// app/admin/professionalbranding/components/CreateChecklistModal.tsx
'use client';

import { useState } from 'react';
import { X, Plus, Trash2 } from 'lucide-react';
import { createBrandingChecklist } from '@/actions/professionalbranding-admin-actions';
...
export function CreateChecklistModal({ isOpen, onClose }: CreateChecklistModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Convert to proper format for the action
      const tasksForSubmission = validTasks.map(task => ({
        title: task.title,
      ...
      const result = await createBrandingChecklist({
        year: formData.year,
      ...
      if (result.success) {
        ...
        alert(`Failed to create checklist: ${result.error}`);
      }
    } catch (error) {
